...
- [x] Phase 1：事件协议对齐（上游 + 前端 contract）
- [x] Phase 2：解析层重构（无抖动、无丢图）
- [ ] Phase 3：头像系统像素对齐（重开，仍需与官方头像资源逐项对齐）
- [ ] Phase 4：思考区交互与滚动行为对齐（重开，需继续压实滚动节奏与收起时机）
- [ ] Phase 5：搜索条目与计数展示对齐（重开，需继续对齐细节文案与节奏）
- [ ] Phase 6：图片结果完整渲染（重开，需完成 Web/Tauri 实机验收）
- [ ] Phase 7：Web/Tauri 一致性与回归（未完成）
...
- 目标：统一使用代理人身份头像，不再把铅笔/声波等工具图标当成代理人头像。
...
- 范围：
  ...
  - 思考中与思考结果都使用代理人身份头像体系。
- 产出：
  - `AgentAvatarStack` 重构与样式 token。
- 验收：
  - 不再出现铅笔/声波/文件夹等工具图标冒充代理人头像。
